...
class SessionManager:
    ...
    def get_effective_jwt_token(self, user_id: str, jwt_token: str) -> str:
        """Get the effective JWT token for the current user.

        API-key users still need a per-user JWT for OpenSearch DLS, even when
        OAuth login is disabled. Falling back to the anonymous JWT first causes
        the public API to search as "anonymous" and lose access to the user's
        own documents.
        """
        ...
        # Real known users should always get a user-scoped JWT, including in
        # no-auth mode where API keys are the primary auth mechanism.
        user = self.get_user(user_id)
        if user:
            return self.create_jwt_token(user)

        # No known user context — anonymous JWT is only correct for genuinely
        # anonymous access or global no-auth browsing.
